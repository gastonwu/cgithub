...
public:    
    ...
    int get(unsigned long _key,valueType &_value);    //if _key in the table,return 0,and set lastFound the position,otherwise return -1
...
template<typename vT,unsigned long maxLine,int lines>
int hash_shm<vT,maxLine,lines>::get(unsigned long _key,vT &_value)
{
    unsigned long hash;
    hash_node *pH=NULL;
    for(int i=0;i<lines;i++)
